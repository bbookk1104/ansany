...
<title>BOOOOP! <%=n.getNoticeTitle() %></title>
</head>
<body>
				...
				<div class="inputRecommentBox">
					<form action="/insertComment.do" method="post">
						<ul>
							...
							<li>
								<input type="hidden" name="ncWriter" value="<%=m.getMemberId() %>">
								<input type="hidden" name="noticeRef" value="<%=nc.getNoticeRef() %>">
	...
	<script>
		...
		$(".recShow").on("click", function(){
			...
			//답글 입력창 직접 클릭하지않고도 자동으로 답글 입력창으로 넘어가는 효과
		});
		function modifyComment(obj,ncNo,noticeNo){
			$(obj).parent().prev().show(); //textarea 화면 보이기
