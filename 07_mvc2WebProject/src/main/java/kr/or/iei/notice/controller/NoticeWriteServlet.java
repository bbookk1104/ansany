...
public class NoticeWriteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//2-1. 파일이 업로드될 경로를 지정
		String root = getServletContext().getRealPath("/"); //webapp폴더의 절대경로 가져옴(c드라이브부터 탐색)
		String saveDirectory = root + "upload/notice";
		//2-2. 파일 업로드 최대용량 설정(일반적인 웹은 최대용량 10MB)
		int maxSize = 10*1024*1024; //10b->10kb->10mb
		//2-3. multipart/form-data에서 데이터를 꺼내기위한 객체변환
		//request -> MultipartRequest객체 변환(cos.jar)
		//매개변수 5개를 전달하면서 객체 생성
		//1)request, 2)파일저장경로, 3)파일최대크기, 4)인코딩타입, 5)파일명중복처리객체 <-5)는 자동제공
		//MultipartRequest객체가 생성되는 시점에 파일 업로드 완료
		...
		//화면에서 첨부한 원본파일이름(중복처리 전 이름)
		String filename = mRequest.getOriginalFileName("upfile");
		//실제 서버에 업로드된 파일이름(중복처리 후 이름) *찐파일
