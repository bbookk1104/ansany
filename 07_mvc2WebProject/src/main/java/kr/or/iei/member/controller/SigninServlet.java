...
public class SigninServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Member m = service.selectOneMember(member);//member:로그인정보-id&pw, m:member와 일치하는 회원정보
		//4. 결과처리
		//4-1. 결과처리할 페이지를 지정
		RequestDispatcher view = request.getRequestDispatcher("/WEB-INF/views/common/msg.jsp");
		if(m!=null) {
			//로그인 성공한 경우
			//회원등급 확인해서 준회원이면 로그인 불가능
			//정회원 또는 관리자면 로그인 처리
			if(m.getMemberLevel()==3) {
				...
				//-> 성공메세지 alert후 메인페이지로 이동
				request.setAttribute("title", "로그인 성공");
				request.setAttribute("msg", "환영합니다!");
				request.setAttribute("icon", "success");
				request.setAttribute("loc", "/");
				//로그인 성공 시 회원정보를 세션에 저장(브라우저 닫기 전까지 정보를 유지하기위함)
				HttpSession session = request.getSession();
				session.setAttribute("m", m);
			}
		}else {
			//로그인 실패한 경우-> 실패메세지 alert후 메인페이지로 이동
